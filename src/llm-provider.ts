      ...
      start(controller) {
        (async () => {
          ...
          try {
            ...
            if (cliPath) {
              if (process.platform === 'win32') {
                // Windows: let SDK self-resolve its bundled binary
                // Don't pass pathToClaudeCodeExecutable
              } else {
            ...
            // ── Case 3: Windows spawn noise (suppress) ──
            // SDK's bundled claude.exe fails to spawn on Windows (EINVAL or exit code 1).
            // The bridge's conversation engine retries automatically and succeeds,
            // so this error is noise. Silently close so the user sees only the retry response.
            if (process.platform === 'win32' && (
                message.includes('spawn EINVAL') ||
                message.includes('spawn EACCES') ||
                (message.includes('process exited with code') && !state.hasReceivedResult)
              )) {
              console.warn('[llm-provider] Suppressing Windows spawn error (bridge will retry):', message.slice(0, 150));
              controller.close();
              return;
            }

            // ── Case 4: Partial output + crash ──
            ...
            } else if (message.includes('native binary') && message.includes('failed to launch')) {
              // Windows: SDK tries to spawn shell script directly which fails.
              // The CLI fallback still works — this error is noise.
              console.warn('[llm-provider] Suppressing native binary launch error (Windows fallback still works)');
              controller.close();
              return;
            } else if (message.includes('spawn') || message.includes('process exited with code')) {
              // Windows: suppress spawn/exit errors - bridge will retry
              console.warn('[llm-provider] Suppressing spawn error (Windows):', message.slice(0, 200));
              controller.close();
              return;
            } else {
              userMessage = message;
